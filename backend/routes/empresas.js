...
router.post('/', auth(['superadmin']), superAdminOnly, async (req, res) => {
  ...
  try {
    ...
    const empResult = await client.query(
      `INSERT INTO empresas (cnpj, nome_fantasia, email, telefone, slug, cor_primaria, vencimento)
...
router.patch('/:id', auth(['superadmin']), superAdminOnly, async (req, res) => {
  const { cnpj, nome_fantasia, email, telefone, slug, cor_primaria, status, vencimento } = req.body;
  try {
    const result = await pool.query(`
      UPDATE empresas SET
      ...
      [cnpj, nome_fantasia, email, telefone, slug?.toLowerCase(), cor_primaria, status, vencimento, req.params.id]
    );
    if (!result.rows.length) return res.status(404).json({ erro: 'Empresa não encontrada' });
    res.json(result.rows[0]);
  } catch (err) { res.status(500).json({ erro: err.message }); }
