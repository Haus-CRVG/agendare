...
function init() {
  ...
  if (linkEl && usuario.slug) {
    const baseUrl = 'https://agendare-backend-production.up.railway.app';
    linkEl.value = `${baseUrl}/frontend/agendar.html?empresa=${usuario.slug}`;
  }
...
function mudarAba(aba) {
  ...
  if (aba==='agenda')          { renderMiniCalendarioComBolinhas(); carregarPainelAgenda(); carregarProximosKanban(); }
...
async function carregarProximosKanban() {
  const container = document.getElementById('proximosKanban');
  if (!container) return;
  try {
    const agora = new Date();
    const ini = agora.toISOString().split('T')[0] + 'T00:00:00';
    const fim = new Date(agora.getFullYear()+2,11,31).toISOString().split('T')[0]+'T23:59:59';
    const r = await fetch(`${API}/agendamentos?inicio=${ini}&fim=${fim}`, { headers:{ Authorization:`Bearer ${token}` } });
    if (!r.ok) return;
    const ags = await r.json();
    if (!ags.length) { container.innerHTML=`<p style="color:var(--muted);padding:1rem;">Nenhum próximo compromisso.</p>`; return; }
  ...
  try {
    ...
    tbody.innerHTML=ags.map(a=>{
      ...
      const dt=ehDiaTodo
        ...
        :new Date(a.data_inicio).toLocaleString('pt-BR',{timeZone:'America/Sao_Paulo',day:'2-digit',month:'2-digit',year:'2-digit',hour:'2-digit',minute:'2-digit'});
      const agora=new Date();
      const dataFim=new Date(a.data_fim||a.data_inicio);
      const atrasado=dataFim<agora&&a.status!=='concluido'&&a.status!=='cancelado';
      const podeEditar=usuario.perfil==='admin'||a.profissional_id===usuario.id;
      return `<tr style="${atrasado?'background:rgba(239,68,68,0.07);':''}">
        <td style="${atrasado?'color:#ef4444;font-weight:600;':''}">${atrasado?'⚠️ ':''}${dt}</td>
        ...
        <td><span class="badge ${statusBadge[a.status]||''}">${a.status}</span>${atrasado?'<span style="margin-left:4px;font-size:0.68rem;background:rgba(239,68,68,0.15);color:#ef4444;border:1px solid rgba(239,68,68,0.3);border-radius:999px;padding:1px 6px;">atrasado</span>':''}</td>
...
async function carregarConfiguracoes(){try{const r=await fetch(`${API}/empresas/minha`,{headers:{Authorization:`Bearer ${token}`}});const emp=await r.json();const linkEl=document.getElementById('linkPublico');if(linkEl){const baseUrl='https://agendare-backend-production.up.railway.app';linkEl.value=`${baseUrl}/frontend/agendar.html?empresa=${emp.slug}`;}}catch(err){console.error(err);}}
...
function iniciarPolling(){verificarNotificacoes();contarAtrasados();pollingInterval=setInterval(()=>{verificarNotificacoes();contarAtrasados();},30000);}
async function contarAtrasados(){
  try{
    const agora=new Date();
    const ini=new Date(agora.getFullYear()-1,0,1).toISOString().split('T')[0]+'T00:00:00';
    const fim=agora.toISOString().split('T')[0]+'T23:59:59';
    const r=await fetch(`${API}/agendamentos?inicio=${ini}&fim=${fim}`,{headers:{Authorization:`Bearer ${token}`}});
    if(!r.ok)return;
    const ags=await r.json();
    const agora2=new Date();
    const atrasados=ags.filter(a=>{
      if(a.status==='concluido'||a.status==='cancelado')return false;
      const dataFim=new Date(a.data_fim||a.data_inicio);
      return dataFim<agora2;
    });
    const badge=document.getElementById('badgeNotif');
    if(badge){
      if(atrasados.length>0){badge.textContent=atrasados.length;badge.style.display='flex';}
